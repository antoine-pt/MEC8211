...
if __name__ == "__main__":
    ...
    # -------------------------------
    # Longueur physique constanteb  
    # -------------------------------
    L = 2e-4   # exemple : 200 µm → doit être = NX * dx

    # -------------------------------
    # Jeux de paramètres
    # -------------------------------
    NX_list   = [50, 75,100,150, 200,400]       # différentes résolutions
    seed_list = [1, 42, 101]         # différentes graines
    ...
    dx_list = []
    for NX in NX_list:
        dx = L / NX   # impose NX * dx = constante
        dx_list.append(dx)
        results = []
        for seed in seed_list:
            print("\n" + "="*60)
            print(f"Simulation avec NX={NX}, dx={dx:.2e}, seed={seed}")
            print("="*60)

            # Génération structure
            d_equivalent = Generate_sample(
                seed, filename, mean_fiber_d, std_d, poro, NX, dx
            )
        ...
        result_moy.append(np.mean([r["k"] for r in results]))


    plt.plot(np.log(dx_list),result_moy)
    plt.xlabel("Log("+r'$\Delta x$' +")")
    plt.ylabel("k")
    plt.title("Graphique de convergence")
    plt.show()
    ...
    p_hat = np.log((f3 - f2) / (f2 - f1)) / np.log(2)
    if abs(pf-p_hat)/pf < 0.1:
        u_num = 1/2*1.25/(r**pf-1)*abs(f2-f1)
    else : 
        p = min(max(0.5,p_hat),pf)
        u_num = 1/2*3/(r**p-1)*abs(f2-f1)
